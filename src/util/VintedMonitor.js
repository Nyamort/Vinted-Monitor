...
const { MessageEmbed, WebhookClient, CommandInteractionOptionResolver } = require('discord.js');
const fs = require('fs');
...
class VintedMonitor{
    ...
    async start(time = 30){
        ...
        //     countReq++;
        //     let proxy = this.proxyList[countReq%this.proxyList.length];
        //     await useProxy(request, `${proxy.host}:${proxy.port}`);
        // });
            ...
            */
            ...
            if(display_products.length){
                ...
                await display_products.forEach(async(id) =>{
                    ...
                    let img = element.photos[0].url ? element.photos[0].url : 'https://upload.wikimedia.org/wikipedia/commons/2/29/Vinted_logo.png';
                    let url = element.url;
